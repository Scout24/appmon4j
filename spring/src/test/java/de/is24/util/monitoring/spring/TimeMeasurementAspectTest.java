...
public class TimeMeasurementAspectTest {
  ...
  public void methodAnnotatedTimerMeasurement() throws Exception {
    ...
    verify(monitorPlugin, times(1)).addTimerMeasurement(eq(
        "de.is24.util.monitoring.spring.MethodAnnotated.methodOne"),
      anyLong());

    verify(monitorPlugin, times(1)).addTimerMeasurement(eq(
        "de.is24.util.monitoring.spring.MethodAnnotated.methodTwo"),
      anyLong());
  ...
  public void classAnnotatedTimerMeasurement() throws Exception {
    ...
    verify(monitorPlugin, times(1)).addTimerMeasurement(eq(
        "de.is24.util.monitoring.spring.ClassAnnotated.methodOne"),
      anyLong());

    verify(monitorPlugin, times(1)).addTimerMeasurement(eq(
        "de.is24.util.monitoring.spring.ClassAnnotated.methodTwo"),
      anyLong());
